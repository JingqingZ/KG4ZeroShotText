...
def create_vocab_given_text(textlist, vocab_path):
    # create dictionary
...
def load_data(filename, vocab_file, processed_file, column, force_process=False):
    ...
    if not force_process and os.path.exists(processed_file) and os.path.exists(vocab_file):
        ...
        vocab = create_vocab_given_text(full_text_list, vocab_path=vocab_file)
...
def build_vocabulary_from_full_corpus(filename, vocab_file, column, force_process=False):
